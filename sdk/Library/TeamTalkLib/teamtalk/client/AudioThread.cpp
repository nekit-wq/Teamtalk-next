...
void AudioThread::SetMicrophoneEqualizer(const std::vector<float>& gains_db)
{
    std::unique_lock<std::recursive_mutex> const g(m_preprocess_lock);
    m_mic_eq_gains_db = gains_db;
    m_mic_eq_state.clear();
    m_mic_eq_state.resize(m_mic_eq_gains_db.size() * 2, std::vector<float>(4, 0.0f));
}

void AudioThread::ApplyMicrophoneEqualizer(media::AudioFrame& audblock)
{
    std::unique_lock<std::recursive_mutex> const g(m_preprocess_lock);
    if (m_mic_eq_gains_db.empty() || audblock.input_buffer == nullptr)
        return;

    const int channels = audblock.inputfmt.channels;
    const float sample_rate = static_cast<float>(audblock.inputfmt.samplerate);
    if (channels <= 0 || sample_rate <= 0)
        return;

    static const float frequencies[] = {60.0f, 120.0f, 250.0f, 500.0f, 1000.0f, 2000.0f, 4000.0f, 8000.0f, 16000.0f};
    const size_t bands = std::min(m_mic_eq_gains_db.size(), sizeof(frequencies) / sizeof(frequencies[0]));
    if (m_mic_eq_state.size() != bands * static_cast<size_t>(channels))
        m_mic_eq_state.resize(bands * static_cast<size_t>(channels), std::vector<float>(4, 0.0f));

    for (size_t band = 0; band < bands; ++band)
    {
        const float gain = m_mic_eq_gains_db[band];
        if (gain == 0.0f)
            continue;
        const float a = std::pow(10.0f, gain / 40.0f);
        const float w0 = 2.0f * 3.14159265358979323846f * frequencies[band] / sample_rate;
        const float alpha = std::sin(w0) / (2.0f * 1.0f);
        ...
        {
            std::vector<float>& state = m_mic_eq_state[band * static_cast<size_t>(channels) + channel];
            for (int sample = 0; sample < audblock.input_samples; ++sample)
            {
                const size_t index = static_cast<size_t>(sample) * channels + channel;
                const float x = static_cast<float>(audblock.input_buffer[index]);
                const float y = nb0 * x + state[0];
                state[0] = nb1 * x - na1 * y + state[1];
                state[1] = nb2 * x - na2 * y;
                audblock.input_buffer[index] = static_cast<short>(std::max(-32768.0f, std::min(32767.0f, y)));
            }
        }
    }
}
...
void AudioThread::ProcessAudioFrame(media::AudioFrame& audblock)
{
    ...
    ApplyMicrophoneEqualizer(audblock);
