...
TEAMTALKDLL_API TTBOOL TT_SetMicrophoneEqualizer(IN TTInstance* lpTTInstance,
                                                  IN const float* lpGainsDB,
                                                  IN INT32 nBands)
{
    clientnode_t clientnode;
    GET_CLIENTNODE_RET(clientnode, lpTTInstance, FALSE);
    if (lpGainsDB == nullptr || nBands < 0 || nBands > 32)
        return FALSE;
    clientnode->SetMicrophoneEqualizer(std::vector<float>(lpGainsDB, lpGainsDB + nBands));
    return TRUE;
}
