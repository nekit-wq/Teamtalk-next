...
extern "C" {
    ...
    JNIEXPORT jboolean JNICALL Java_dk_bearware_TeamTalkBase_setMicrophoneEqualizer(JNIEnv* env,
                                                                                   jobject thiz,
                                                                                   jfloatArray gains)
    {
        if (gains == nullptr)
        ...
        if (count > 32)
            return JFALSE;
        jfloat* values = env->GetFloatArrayElements(gains, nullptr);
        if (values == nullptr)
            return JFALSE;
        TTBOOL result = TT_SetMicrophoneEqualizer(GetTTInstance(env, thiz), values, static_cast<INT32>(count));
        env->ReleaseFloatArrayElements(gains, values, JNI_ABORT);
        return result ? JTRUE : JFALSE;
    }
